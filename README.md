...
- 회원가입 기능 구현
- 일반 로그인 및 소셜 로그인 (카카오톡) 기능
- JWT(Access Token, Refresh Token) 관리
- 상세 페이지 기능: 게시글 조회, '나도 불편해요', '해결했어요' 기능, 게시글 삭제 기능
- 댓글 반응형 UI
- '해결했어요' 누적 시 완료 게시글 구현
- 마이페이지: 유저 정보 view 반응형 UI, 유저 정보 조회, 이메일 추가 등록, 닉네임 변경, 비밀번호 변경, 회원 탈퇴 기능
- 404 페이지 구현
- 범용적으로 사용되는 api instance 및 interceptor, 모달, 버튼, Media Query, Global Theme, Toast 알람 등 구현
